...
Two sibling unbounded loops fall under the same monitoring control (maintenance reference): the per-operator **vault** iteration in `_calculateAndStoreVaultShares` (bound registered vaults per class — `registerVault` is governance-gated) and the middleware's whole-set cache catch-up `_processSingleEpochNodeStakeCacheUpdate` (bound operator count and keep a keeper SLA of one stake-cache sync per epoch so the manual multi-epoch path is never needed at scale).
