...
} from "@suzaku/contracts-library/contracts/ValidatorManager/BalancerValidatorManager.sol";
import {PoASecurityModule} from
    "@suzaku/contracts-library/contracts/ValidatorManager/SecurityModule/PoASecurityModule.sol";
    ...
    function run(
        ...
        if (deployPoASecurityModule) {
            initialSecurityModule =
                vm.computeCreateAddress(proxyAdminOwnerAddress, vm.getNonce(proxyAdminOwnerAddress) + 2);
        }
        ...
        if (deployPoASecurityModule) {
            securityModuleDeploymentAddress = address(new PoASecurityModule(proxy, validatorManagerOwnerAddress));
