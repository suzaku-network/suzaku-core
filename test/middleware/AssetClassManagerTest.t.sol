...
contract AssetClassRegistryTest is Test {
    ...
    function setUp() public {
        ...
        // with min = 10, max = 0
        assetClassRegistry.addAssetClass(2, 10, 0);
    }
    ...
    function test_DefaultAssetIsInClass1() public view {
        address[] memory assets = assetClassRegistry.getAssets(1);
    ...
    function test_RevertOnRemoveDefaultAssetFromClass1() public {
        ...
        assetClassRegistry.removeAsset(1, tokenA);
    }

    function test_AddAssetToClass1() public {
        // Add something other than the default asset
        assetClassRegistry.addAsset(1, tokenB);
        address[] memory assets = assetClassRegistry.getAssets(1);
    ...
    function test_MultipleAssetsInClass1() public {
        assetClassRegistry.addAsset(1, tokenB);
        assetClassRegistry.addAsset(1, tokenC);
    ...
    function test_AddAssetToClass2() public {
        assetClassRegistry.addAsset(2, tokenB);
        address[] memory assets = assetClassRegistry.getAssets(2);
        assertEq(assets.length, 1, "Expected 1 asset in class 2");
        assertEq(assets[0], tokenB, "Expected asset to match tokenB");
    }

    function test_RemoveAssetFromClass1() public {
        // Add an asset (alice) to class #1
        assetClassRegistry.addAsset(1, tokenB);

        // Remove alice (allowed because she's not the default asset)
        assetClassRegistry.removeAsset(1, tokenB);
    ...
    function test_RemoveAssetFromClass2() public {
        assetClassRegistry.addAsset(2, tokenB);
        assetClassRegistry.removeAsset(2, tokenB);
        address[] memory assets = assetClassRegistry.getAssets(2);
        assertEq(assets.length, 0, "Expected no assets in class 2 after removal");
    }

    function test_AddAssetClassAndCheckStakes() public {
        // Add new class #3
        assetClassRegistry.addAssetClass(3, 123, 456);
    ...
    function test_RevertOnDuplicateAssetClass() public {
        ...
        assetClassRegistry.addAssetClass(1, 123, 456);
    }

    function test_RevertOnGetAssetsForNonexistentClass() public {
        vm.expectRevert(IAssetClassRegistry.AssetClassRegistry__AssetClassNotFound.selector);
        assetClassRegistry.getAssets(999);
    }

    function test_RevertOnRemoveAssetFromInvalidClass() public {
        vm.expectRevert(IAssetClassRegistry.AssetClassRegistry__AssetClassNotFound.selector);
        assetClassRegistry.removeAsset(999, alice);
    }

    function test_RevertOnRemoveNonexistentAsset() public {
        assetClassRegistry.addAsset(1, alice);
        // Try remove bob from class 1
        vm.expectRevert(IAssetClassRegistry.AssetClassRegistry__AssetNotFound.selector);
        assetClassRegistry.removeAsset(1, bob);
    }
