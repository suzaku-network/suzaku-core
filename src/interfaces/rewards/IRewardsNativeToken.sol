...
/**
 * @title IRewardsNativeToken
 * @notice Interface for managing distribution and claiming of staking rewards with single native token support
 */
interface IRewardsNativeToken {
    // ============================
    //         ERRORS
    // ============================
    /**
     ...
     * @param recipient Address receiving the curator fee
     */
    function claimCuratorFee(address recipient) external;

    /**
     * @notice Claims accumulated operator fees
     * @param recipient Address receiving the operator fee
     */
    function claimOperatorFee(address recipient) external;
     ...
     * @param recipient Address receiving the protocol fee
     */
    function claimProtocolFee(address recipient) external;
     ...
     * @param recipient Address receiving the undistributed rewards
     */
    function claimUndistributedRewards(uint48 epoch, address recipient) external;
    ...
    /**
     * @notice Retrieves the rewards amount for a given epoch
     * @dev This function allows external callers to view the rewards amount for a specific epoch
     * @param epoch The epoch for which to fetch reward amount
     * @return amount The reward amount for the epoch
     */
    function getEpochRewards(uint48 epoch) external view returns (uint256 amount);

    /**
     * @notice Sole rewards token address
     * @return The address of the native rewards token
     */
