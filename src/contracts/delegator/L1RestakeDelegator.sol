...
contract L1RestakeDelegator is BaseDelegator, IL1RestakeDelegator {
    ...
    function l1LimitAt(
        ...
        return _l1Limit[l1][collateralClass].upperLookupRecent(timestamp, hint);
    }

    /**
     * @inheritdoc IL1RestakeDelegator
     */
    function l1Limit(address l1, uint96 collateralClass) public view returns (uint256) {
        return _l1Limit[l1][collateralClass].latest();
    }
    ...
    function totalOperatorL1SharesAt(
        ...
        return _totalOperatorL1Shares[l1][collateralClass].upperLookupRecent(timestamp, hint);
    }

    /**
     * @inheritdoc IL1RestakeDelegator
     */
    function totalOperatorL1Shares(address l1, uint96 collateralClass) public view returns (uint256) {
        return _totalOperatorL1Shares[l1][collateralClass].latest();
    }
    ...
    function operatorL1SharesAt(
        ...
        return _operatorL1Shares[l1][collateralClass][operator].upperLookupRecent(timestamp, hint);
    }

    /**
     * @inheritdoc IL1RestakeDelegator
     */
    function operatorL1Shares(address l1, uint96 collateralClass, address operator) public view returns (uint256) {
        return _operatorL1Shares[l1][collateralClass][operator].latest();
    }

    /**
     * @inheritdoc IL1RestakeDelegator
     */
    function setL1Limit(address l1, uint96 collateralClass, uint256 amount) external onlyRole(L1_LIMIT_SET_ROLE) {
        uint256 maxLimit = maxL1Limit[l1][collateralClass];
        if (maxLimit == 0) {
        ...
        if (l1Limit(l1, collateralClass) == amount) {
            revert BaseDelegator__AlreadySet();
        }

        _l1Limit[l1][collateralClass].push(Time.timestamp(), amount);

        emit SetL1Limit(l1, collateralClass, amount);
    }
    ...
    function setOperatorL1Shares(
        ...
        uint256 operatorL1Shares_ = operatorL1Shares(l1, collateralClass, operator);
        if (operatorL1Shares_ == shares) {
            revert BaseDelegator__AlreadySet();
        }

        _totalOperatorL1Shares[l1][collateralClass].push(
            Time.timestamp(), totalOperatorL1Shares(l1, collateralClass) - operatorL1Shares_ + shares
        );
        _operatorL1Shares[l1][collateralClass][operator].push(Time.timestamp(), shares);

        emit SetOperatorL1Shares(l1, collateralClass, operator, shares);
    }
    ...
    function _stakeAt(
        ...
        uint256 totalOperatorL1SharesAt_ =
            totalOperatorL1SharesAt(l1, collateralClass, timestamp, stakesHints.totalOperatorL1SharesHint);
        return totalOperatorL1SharesAt_ == 0
            ? (0, stakesHints.baseHints)
            : (
                operatorL1SharesAt(l1, collateralClass, operator, timestamp, stakesHints.operatorL1SharesHint).mulDiv(
                    Math.min(
                        IVaultTokenized(vault).activeStakeAt(timestamp, stakesHints.activeStakeHint),
                        l1LimitAt(l1, collateralClass, timestamp, stakesHints.l1LimitHint)
                    ),
    ...
    function _stake(address l1, uint96 collateralClass, address operator) internal view override returns (uint256) {
        uint256 totalOperatorL1Shares_ = totalOperatorL1Shares(l1, collateralClass);
        return totalOperatorL1Shares_ == 0
            ? 0
            : operatorL1Shares(l1, collateralClass, operator).mulDiv(
                Math.min(IVaultTokenized(vault).activeStake(), l1Limit(l1, collateralClass)), totalOperatorL1Shares_
            );
    }

    function _setMaxL1Limit(address l1, uint96 collateralClass, uint256 amount) internal override {
        (bool exists,, uint256 latestValue) = _l1Limit[l1][collateralClass].latestCheckpoint();
